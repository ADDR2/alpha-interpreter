...
function FindScope(name, index){
    ...
    if (index >= 0){
        if (name in Stack.getByIndex(index)){
            return Stack.getByIndex(index);
        }
...
function Value(astNode){
    ...
    if(astNode.type == 'Var'){
        if(!(astNode.name in FindScope(astNode.name, Stack.size()-1))) {
            console.log("Error en la linea " + (astNode.line+1) + ": La variable "+astNode.name+" no existe");
            return null;
        }else{
            astNode = new AstNode(FindScope(astNode.name, Stack.size()-1)[astNode.name].type, {value : FindScope(astNode.name, Stack.size()-1)[astNode.name].value});
        }
    }
    if (astNode.type == 'Array'){
        if(!(astNode.name in FindScope(astNode.name, Stack.size()-1))) {
            ...
            var rgid = FindScope(astNode.name, Stack.size()-1)[astNode.name];
            if(astNode.dims == rgid.dim){
                ...
                var help = FindScope(rgid.of, Stack.size()-1);
                if (rgid.of in help){
    ...
    if (astNode.type == 'Register'){
        if(!(astNode.name in FindScope(astNode.name, Stack.size()-1))) {
            ...
            var alias = findItem(astNode.access, FindScope(astNode.name, Stack.size()-1));
...
function PointerValue(astNode){
    if (astNode.type == 'Var'){
        return FindScope(astNode.name, Stack.size()-1)[astNode.name];
    }
    if(FindScope(astNode.name, Stack.size()-1)[astNode.name].type == 'Register'){
        return findItem(astNode.access, FindScope(astNode.name, Stack.size()-1));
    }
}
...
function FindType(type){
    ...
    for (var attr in FindScope(type, Stack.size()-1)[type]) {
           if (FindScope(type, Stack.size()-1)[type].hasOwnProperty(attr)) copy[attr] = FindScope(type, Stack.size()-1)[type][attr];
...
function GiveType(type){
    return FindScope(type, Stack.size()-1)[type];
}
...
class SemanticProcessor {
    constructor(nodeTypes = Statements) {
    ...
    getValueAndType(astNode) {
        if(astNode.type in Types) {
            return { type: astNode.type, node: astNode };
        } else if(astNode.type === 'Var') {
            const getVar = this.FindScope(astNode.name, this.executionStack.size()-1);

            return { type: getVar.type, name: getVar.name, node: getVar };
        } else if(astNode.type === 'Register') {
            return findItem(astNode.access, this.FindScope(astNode.name, this.executionStack.size()-1));
        }

        return {};
    }

    FindScope(name, chunkIndex) {
        if (chunkIndex >= 0){
            const currentScope = this.Stack.getByIndex(chunkIndex); 
            return name in currentScope? currentScope : FindScope(name, chunkIndex-1);
        }

        throw new Error(`Identifier ${name} is not defined`);
    }

    notInCurrentScope(name) {
        const currentScope = this.executionStack.top();
        return !(name in currentScope) && currentScope;
    }

    eval(astNode = null) {
        if(astNode === null || !('type' in astNode) || !this.nodeTypes[astNode.type])
            throw new Error('Invalid astNode found');
...
function eval(astNode) {
    ...
    switch(astNode.type) {
        ...
        case 'Next Declaration':
            var lf = eval(astNode.left);
            if (aux_type in FindScope(aux_type, Stack.size()-1) && !(astNode.name in Stack.top())){
                Stack.top()[astNode.name] = FindType(aux_type);
                if (lf != null){
                    lf = Value(lf);
                    if(Stack.top()[astNode.name].type == lf.type){
                        console.log("buena declaracion");
                    }else{
                        if (Stack.top()[astNode.name].type == 'Integer' && lf.type == 'Real'){
                            lf.value = parseInt(lf.value);
                            lf.type = 'Integer';
                            console.log("buena transformacion a Integer "+lf.value);
                        }else{
                            if (Stack.top()[astNode.name].type == 'Real' && lf.type == 'Integer'){
                    ...
                    Stack.top()[astNode.name].value = lf.value;
                    console.log("valor "+Stack.top()[astNode.name].value);
                }
            }else{
                if (aux_type == 'Integer' || aux_type == 'Char' || aux_type == 'String' || aux_type == 'Boolean' || aux_type == 'Real'){
                    if (lf != null){
                        ...
                        Stack.top()[astNode.name] = new AstNode(aux_type, {value : null, origin: astNode.name});
                        console.log("La variable "+astNode.name+" no tiene valor");
                    }
                }else{
        ...
        case 'Asig':
            var lf = eval(astNode.left);
            if (astNode.ref){
                if(!(lf.name in FindScope(lf.name, Stack.size()-1)) || FindScope(lf.name, Stack.size()-1)[lf.name].type != 'Pointer') {
                    console.log("Error en la linea " + (astNode.line+1) + ": El apuntador "+lf.name+" no ha sido declarado");
                    return null;
                }else{
                    lf.name = FindScope(lf.name, Stack.size()-1)[lf.name].name;
                    if (lf.type == 'Register'){
                        var i = 0;
                        while(lf.access[i] != '.')
                            i++;
            ...
            if(lf.name in FindScope(lf.name, Stack.size()-1)) {
                var rg = eval(astNode.right);
                if (FindScope(lf.name, Stack.size()-1)[lf.name].type == 'Pointer'){
                    if (rg.type == 'Pointer'){
                        FindScope(lf.name, Stack.size()-1)[lf.name].dir = null;
                        FindScope(lf.name, Stack.size()-1)[lf.name].name = null;
                    }else{
                        if(FindScope(lf.name, Stack.size()-1)[lf.name].pt == PointerValue(rg).type || FindScope(lf.name, Stack.size()-1)[lf.name].pt == PointerValue(rg).origin){
                            FindScope(lf.name, Stack.size()-1)[lf.name].dir = PointerValue(rg);
                            FindScope(lf.name, Stack.size()-1)[lf.name].name = rg.name;
                        }else{
                            if (PointerValue(rg).type == 'Pointer' && PointerValue(rg).pt == FindScope(lf.name, Stack.size()-1)[lf.name].pt){
                                FindScope(lf.name, Stack.size()-1)[lf.name] = PointerValue(rg);
                            }else{
                                console.log("Error en la linea " + (astNode.line+1) + ": Mala asignacion de apuntador");
                                return null;
                            }
                    ...
                    var id = FindScope(lf.name, Stack.size()-1)[lf.name];
                    if (lf.type == 'Var'){
                        rg = Value(rg);
                        if (id.type == rg.type){
                            FindScope(lf.name, Stack.size()-1)[lf.name].value = rg.value;
                        }else{
                            if(id.type == 'Integer' && rg.type == 'Real'){
                                FindScope(lf.name, Stack.size()-1)[lf.name].value = parseInt(rg.value);
                            }else{
                                if(id.type == 'Real' && rg.type == 'Integer'){
                                    FindScope(lf.name, Stack.size()-1)[lf.name].value = parseFloat(rg.value);
                                }else{
                        ...
                        if (lf.type == 'Array'){
                            rg = Value(rg);
                            if(lf.dims == id.dim && FindScope(lf.name, Stack.size()-1)[lf.name].type == 'Array'){
                                var formula = 0;
                                var aux = 1;
                                var cont = id.dim;
                                ...
                                aux_type = id.of;
                                if (id.of in FindScope(id.of, Stack.size()-1)){
                                    FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = FindScope(id.of, Stack.size()-1)[id.of];
                                    aux_type = FindScope(id.of, Stack.size()-1)[id.of].type;
                                }
                                if(aux_type == 'Register'){
                                    if (findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == rg.type){
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = rg.value;
                                    }else{
                                        if(findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'Integer' && rg.type == 'Real'){
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = parseInt(rg.value);
                                        }else{
                                            if(findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'Real' && rg.type == 'Integer'){
                                                findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = parseFloat(rg.value);
                                            }else{
                                    ...
                                    id.of = aux_type;
                                    if (id.of == rg.type){
                                        FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = rg.value;
                                    }else{
                                        if(id.of == 'Integer' && rg.type == 'Real'){
                                            FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = parseInt(rg.value);
                                        }else{
                                            if(id.of == 'Real' && rg.type == 'Integer'){
                                                FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = parseFloat(rg.value);
                                            }else{
                                    ...
                                    console.log(lf.name+"["+formula+"] = "+FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]);
                                }
                            }else{
                            ...
                            if (lf.type == 'Register'){
                                var temp = findItem(lf.access, FindScope(lf.name, Stack.size()-1));
                                if (temp.type == 'Pointer'){
                                    if (rg.type == 'Pointer'){
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)).dir = null;
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)).name = null;
                                    }else{
                                        if (temp.pt == PointerValue(rg).type || temp.pt == PointerValue(rg).origin){
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)).dir = PointerValue(rg);
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)).name = rg.name;
                                        }else{
                                            if(PointerValue(rg).type == 'Pointer' && temp.pt == PointerValue(rg).pt){
                                                findItem(lf.access, FindScope(lf.name, Stack.size()-1)) = PointerValue(rg);
                                            }else{
                                                console.log("Error en la linea " + (astNode.line+1) + ": Mala asignacion de apuntador");
                                                return null;
                                            }
                                    ...
                                    if (temp.type == rg.type){
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = rg.value;
                                    }else{
                                        if(temp.type == 'Integer' && rg.type == 'Real'){
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = parseInt(rg.value);
                                        }else{
                                            if(temp.type == 'Real' && rg.type == 'Integer'){
                                                findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = parseFloat(rg.value);
                                            }else{
                                                console.log("Error en la linea " + (astNode.line+1) + ": El campo del registro "+lf.name+" no es del tipo "+rg.type);
                                                return null;		
                                            }
                                ...
                                console.log("El valor guardado es "+findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value);
                            }else{
        ...
        case 'Register':
            if(!(astNode.name in Stack.top())) {
                ...
                eval(astNode.left);
                Stack.top()[astNode.name] = new AstNode('Register', {elements: list, origin: astNode.name});
                aux_type = "";
                console.log("El registro "+astNode.name+" tiene "+Object.size(Stack.top()[astNode.name].elements)+" elementos");
            }else{
        ...
        case 'List':
            if(!(astNode.right in list)) {
                ...
                if(lf != 'Integer' && lf != 'Real' && lf != 'String' && lf != 'Char' && lf != 'Boolean'){
                    if(lf in FindScope(lf, Stack.size()-1)) {
                        if (astNode.point){
        ...
        case 'Array':
            ...
            if(!(astNode.name in Stack.top())) {
                if(list.length <= 0){
                    ...
                    if(astNode.tov == 'Integer' || astNode.tov == 'Real' || astNode.tov == 'String' || astNode.tov == 'Char' || astNode.tov == 'Boolean' || astNode.tov in FindScope(astNode.tov, Stack.size()-1)){
                        Stack.top()[astNode.name] = new AstNode('Array', {of : astNode.tov, array : element, dim : dim.type, dims : dimen, size : tam});
                    }else{
                    ...
                    var tam = list.length;
                    Stack.top()[astNode.name] = new AstNode('Array', {of : aux_type, array : list, dim : 1, dims : dimen, size : tam, origin: astNode.name});
        ...
        case 'Function':
            if(!(astNode.name in Stack.top())) {
                list = [];
                eval(astNode.left);
                Stack.top()[astNode.name] = new AstNode('Function', {chunk: astNode.right, params: list, ret_type : astNode.ret});
                console.log("La accion "+astNode.name+" tiene "+Object.size(Stack.top()[astNode.name].params)+" parametros");
            }else{
        ...
        case 'Void':
            if(!(astNode.name in Stack.top())) {
                list = [];
                eval(astNode.left);
                Stack.top()[astNode.name] = new AstNode('Void', {chunk: astNode.right, params: list});
                console.log("La accion "+astNode.name+" tiene "+Object.size(Stack.top()[astNode.name].params)+" parametros");
            }else{
                console.log("Error en la linea " + (astNode.line+1) + ": Ya existe el nombre "+astNode.name);
                return null;
        ...
        case 'Call':
            ...
            if(!(lf.name in FindScope(lf.name, Stack.size()-1))) {
                ...
                if (FindScope(lf.name, Stack.size()-1)[lf.name].type == 'Function' || FindScope(lf.name, Stack.size()-1)[lf.name].type == 'Void'){
                    var func = FindScope(lf.name, Stack.size()-1)[lf.name];
                    ...
                    console.log("Error en la linea " + (astNode.line+1) + ": La variable "+lf.name+" no es del tipo funcion/accion, es "+FindScope(lf.name, Stack.size()-1)[lf.name].type);
                    return null;
                }
            }
        break;
                ...
                if(id.type != 'Pointer'){
                    ...
                    if (lf.type == 'Var'){
                        if (id.type == GetType(ent[0])){
                            ...
                            FindScope(lf.name, Stack.size()-1)[lf.name].value = value;
                        }else{
                            if(id.type == 'Integer' && GetType(ent[0]) == 'Real'){
                                FindScope(lf.name, Stack.size()-1)[lf.name].value = parseInt(ent[0]);
                            }else{
                                if(id.type == 'Real' && GetType(ent[0]) == 'Integer'){
                                    FindScope(lf.name, Stack.size()-1)[lf.name].value = parseFloat(ent[0]);
                                }else{
                                    if (id.type == 'Char' && ent[0].length == 1){
                                        FindScope(lf.name, Stack.size()-1)[lf.name].value = ent[0][0];
                                    }else{
                                        if (id.type == 'String'){
                                            FindScope(lf.name, Stack.size()-1)[lf.name].value = ent[0];
                                        }else{
                        ...
                        if (lf.type == 'Array'){
                            if(lf.dims == id.dim && FindScope(lf.name, Stack.size()-1)[lf.name].type == 'Array'){
                                ...
                                if(aux_type == 'Register'){
                                    if (findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == GetType(ent[0])){
                                        var value;
                                        if (ent[0] == 'true') value = true;
                                        if (ent[0] == 'false') value = false;
                                        ...
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = value;
                                    }else{
                                        if(findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'Integer' && GetType(ent[0]) == 'Real'){
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = parseInt(ent[0]);
                                        }else{
                                            if(findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'Real' && GetType(ent[0]) == 'Integer'){
                                                findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = parseFloat(ent[0]);
                                            }else{
                                                if (findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'Char' && ent[0].length == 1){
                                                    findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = ent[0][0];
                                                }else{
                                                    if (findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).type == 'String'){
                                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)[lf.name].array[formula]).value = ent[0];
                                                    }else{
                                    ...
                                    if (id.of == GetType(ent[0])){
                                        ...
                                        FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = value;
                                    }else{
                                        if(id.of == 'Integer' && GetType(ent[0]) == 'Real'){
                                            FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = parseInt(ent[0]);
                                        }else{
                                            if(id.of == 'Real' && GetType(ent[0]) == 'Integer'){
                                                FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = parseFloat(ent[0]);
                                            }else{
                                                if (id.of == 'Char' && ent[0].length == 1){
                                                    FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = ent[0][0];
                                                }else{
                                                    if (id.of == 'String'){
                                                        FindScope(lf.name, Stack.size()-1)[lf.name].array[formula] = ent[0];
                                                    }else{
                            ...
                            if (lf.type == 'Register'){
                                ...
                                if (temp.type == 'Pointer'){
                                    ...
                                    if (temp.type == GetType(ent[0])){
                                        ...
                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = value;
                                    }else{
                                        if(temp.type == 'Integer' && GetType(ent[0]) == 'Real'){
                                            findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = parseInt(ent[0]);
                                        }else{
                                            if(temp.type == 'Real' && GetType(ent[0]) == 'Integer'){
                                                findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = parseFloat(ent[0]);
                                            }else{
                                                if (temp.type == 'Char' && ent[0].length == 1){
                                                    findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = ent[0][0];
                                                }else{
                                                    if (temp.type == 'String'){
                                                        findItem(lf.access, FindScope(lf.name, Stack.size()-1)).value = ent[0];
                                                    }else{
        ...
        case 'For':
            ...
            if(lf.name in FindScope(lf.name, Stack.size()-1)) {
                ...
                if (id.type == rg.type && id.type == inc.type && id.type == end.type && id.type == 'Integer' ){
                    var stop = false;
                    FindScope(lf.name, Stack.size()-1)[lf.name].value = rg.value;
                    while (FindScope(lf.name, Stack.size()-1)[lf.name].value != end.value && !stop){
                        ...
                        FindScope(lf.name, Stack.size()-1)[lf.name].value += inc.value;
                    }
                }else{
        ...
        break;
        case 'Foreach':
            if(!(astNode.element in FindScope(astNode.element, Stack.size()-1))) {
                if(!(astNode.array in FindScope(astNode.array, Stack.size()-1))) {
                    console.log("Error en la linea " + (astNode.line+1) + ": El arreglo "+astNode.array+" no ha sido declarado");
                    return null;
                }else{
                    var arr = FindScope(astNode.array, Stack.size()-1)[astNode.array];
                    if (arr.type == 'Array'){
                        var stop = false;
                        for (var i = 0; i < arr.size && !stop; i++){
                            if (arr.of in FindScope(arr.of, Stack.size()-1)){
                                Stack.top()[astNode.element] = FindType(arr.of);
                            }else{
                                Stack.top()[astNode.element] = new AstNode(FindScope(astNode.array, Stack.size()-1)[astNode.array].of, {value: FindScope(astNode.array, Stack.size()-1)[astNode.array].array[i]});
                            }
                            ...
                            if (arr.of in FindScope(arr.of, Stack.size()-1)){
                                FindScope(astNode.array, Stack.size()-1)[astNode.array].array[i] = Stack.top()[astNode.element];
                            }else{
                                FindScope(astNode.array, Stack.size()-1)[astNode.array].array[i] = Stack.top()[astNode.element].value;
                            }

                            console.log("El valor es "+FindScope(astNode.array, Stack.size()-1)[astNode.array].array[i]);
                        }
                        delete Stack.top()[astNode.element];
                    }else{
                        console.log("Error en la linea " + (astNode.line+1) + ": La variable "+astNode.array+" no es de tipo arreglo, es de tipo "+FindScope(astNode.array, Stack.size()-1)[astNode.array].type);
                        return null;
                    }
                }
            }else{
        ...
        case '==': 
            ...
            var rg = eval(astNode.right);
            if (lf.type == 'Pointer'){
                if(!(rg.name in FindScope(rg.name, Stack.size()-1))) {
                    console.log("Error en la linea " + (astNode.line+1) + ": La variable "+rg.name+" no existe");
                    return null;
                }else{
                    if (rg.type == 'Register'){
                        v = new AstNode('Boolean', {value : null == findItem(rg.access, FindScope(rg.name, Stack.size()-1)).dir});
                    }else{
                        v = new AstNode('Boolean', {value : null == FindScope(rg.name, Stack.size()-1)[rg.name].dir});
                    }
                ...
                if (rg.type == 'Pointer'){
                    if(!(lf.name in FindScope(lf.name, Stack.size()-1))) {
                        ...
                        if (lf.type == 'Register'){
                            v = new AstNode('Boolean', {value : null == findItem(lf.access, FindScope(lf.name, Stack.size()-1)).dir});
                        }else{
                            v = new AstNode('Boolean', {value : null == FindScope(lf.name, Stack.size()-1)[lf.name].dir});
                        }
